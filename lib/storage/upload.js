...
/**
 * Upload event illustration/image to Supabase Storage
 * Used during event creation (with userId for temp path)
...
export async function uploadEventImage(userId, file) {
  try {
    ...
    if (!userId) {
      return {
        data: null,
        error: { message: 'User ID is required for event image upload' }
      }
    }

    // Use temp path during event creation (eventId not yet available)
    const timestamp = Date.now()
    const fileName = `temp/${userId}/${timestamp}-${file.name}`
