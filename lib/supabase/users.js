...
export async function createUserProfile(userId, profileData, email) {
  ...
  try {
    ...
    // Extract school from email domain (e.g., "user@stanford.edu" -> "stanford.edu")
    // The school field is required by the User table schema
    let school = null
    if (email && email.includes('@')) {
      const domain = email.split('@')[1] // Get domain part after @
      if (domain) {
        school = domain.toLowerCase() // Store full domain as school (e.g., "stanford.edu")
      }
    }
    
    // If we couldn't extract school, use a default (shouldn't happen with .edu emails)
    if (!school) {
      school = 'unknown.edu'
    }
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/3bf1bc05-78e8-4bdd-bb3f-5c49e2efc81a',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'users.js:125',message:'Before insert attempt',data:{userId,email,school,name:profileData.name,year:profileData.year,gender:profileData.gender,hasProfilePic:!!profileData.profile_pic,timestamp:now},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
    // #endregion
    ...
    const { data: user, error: userError } = await supabase
      ...
      .insert({
        ...
        school: school, // Extract school from email domain
        name: profileData.name,
        year: profileData.year,
        gender: profileData.gender,
        profile_pic: profileData.profile_pic,
