 ...
 */
export async function getPeopleYouMet(userId, limit = 20) {
  try {
    const supabase = await createClient()
    ...
    otherCheckins?.forEach(checkin => {
      if (!excludedUserIds.has(checkin.user_id)) {
        ...
        if (checkin.checked_in_at > userData.lastEventDate) {
          userData.lastEventDate = checkin.checked_in_at
        }
      }
    })

    // Get user profiles for suggested users
    const suggestedUserIds = Array.from(userEventMap.keys()).slice(0, limit)
    ...
    const { data: users, error: usersError } = await supabase
      .from('User')
      .select('id, name, profile_pic, year')
    ...
    // Combine user data with event counts
    const suggestions = users
      .map(user => {
        ...
        return {
          ...
          lastEventDate: userData?.lastEventDate || null
        }
      })
      .sort((a, b) => {
        // Sort by shared events count (desc), then by last event date (desc)
