...
export async function autoLinkUser(userId, email) {
  try {
    ...
    // IMPORTANT: This checks the database first, so if migrations have populated
    // correct school names, those will be used. If auto-detection creates a school
    // with an incorrect name (e.g., "Berkeley University"), the migration script
    // (010_add_test_schools.sql) will correct it to the proper name.
    ...
    // IMPORTANT: Only create if school truly doesn't exist
    // The getSchoolByDomain function already checked, but double-check to avoid race conditions
    if (!school) {
      // Generate school name from domain (e.g., "stanford.edu" → "Stanford University")
      // This is a best-guess fallback - ideally schools should be pre-populated via migration
      // Note: This auto-generated name may not be accurate (e.g., "Berkeley University" 
      // instead of "University of California, Berkeley"). The migration script should be
      // run first to populate correct school names.
      ...
      if (createError) {
        // If creation fails due to duplicate (race condition), try to fetch existing school
        if (createError.code === '23505') { // Unique violation
          const { data: existingSchool } = await getSchoolByDomain(domain)
          if (existingSchool) {
